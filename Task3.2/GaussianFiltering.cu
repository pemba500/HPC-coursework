...
int main(int argc, char **argv){
	...
	const char* newFileName = "filtered_img.png";
	
	
	error = lodepng_decode32_file(&img, &image_width, &image_height, filename);
	if(error){
	...
	const int ARRAY_SIZE = image_width*image_height*4;
	...
	// launch the kernel
	Gaussian_filtering<<< image_height-2, image_width-2 >>>(d_out, d_in);
	...
	encError = lodepng_encode32_file(newFileName, host_imageOutput, image_width, image_height);
	if(encError){
